...
pub struct FaceConfig {
    ...
    /// Path to the SEC1 P-521 public key (133 B raw) used to seal the ESP
    /// face mirror.  Generated by `/face keygen` or `/definehome`.
    /// Empty means "no sealed mirror" — `mirror_to_esp_sealed` is a no-op.
    #[serde(default)]
    pub seal_pub_ecdh_path: String,
    /// Path to the ML-KEM-1024 encapsulation key (raw bytes) used together
    /// with the P-521 key to seal the ESP face mirror.
    #[serde(default)]
    pub seal_pub_kem_path: String,
...
impl Default for FaceConfig {
    fn default() -> Self {
        Self {
            ...
            seal_pub_ecdh_path: String::new(),
            seal_pub_kem_path: String::new(),
