...
| **Device watcher** | A keyboard or a disk that appears after the owner accepted a baseline raises an alert on its own — a way to type that did not exist a minute ago, or a disk attached to an unattended machine, which is a copy rather than a visit. Spoken in the configured persona like every other passive alert |
